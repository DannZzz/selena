...
import { read, AUTO, MIME_PNG, BLEND_MULTIPLY } from "jimp";
import { createCanvas, registerFont } from "canvas";
import { readdirSync } from "fs";
import { DiscordUser } from "../structures/MainTypes";
import { Functions } from "../structures/Functions";

const { moment } = Functions;

  registerFont(`${process.cwd()}/assets/discord-arts/Fonts/Helvetica.ttf`, {
    family: `Helvetica Normal`,
  });
  registerFont(`${process.cwd()}/assets/discord-arts/Fonts/Helvetica-Bold.ttf`, {
    family: `Helvetica Bold`,
  });

export default async function profileImage(userParam: DiscordUser){
    ...
    const base = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/UserBase.png`);
    const capa = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/UserProfile.png`);
    const mask = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/mask.png`);
    const mark = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/mark.png`);

    const empty = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/Status/empty.png`);
    ...
    if(user.displayAvatarURL({forceStatic: false})?.endsWith(".gif") || user.bannerURL()){
        const boostBadges = readdirSync(`${process.cwd()}/assets/discord-arts/Images/profileImage/Badges/Boost`).filter((file) => file.endsWith(".png") );
        const boostRandom = boostBadges[Math.floor(Math.random() * boostBadges.length)]
        const nitro = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/Badges/v14/Nitro.png`)
        const boost = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/Badges/Boost/${boostRandom}`)
    ...
    mark.opacity(0.5);
    (base as any).composite(mark, 0, 0, {mode: BLEND_MULTIPLY as any});
    (base as any).mask(mask);
    ...
    if(!user.bot){
        ...
        for (let i = 0; i < flags.length; i++) {
            let badge = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/Badges/v14/${flags[i]}.png`);
            badges.push(badge);
          }
        ...
        if (flags.includes("VerifiedBot")) {
          botBagde = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/botVerif.png`);
        } else {
          botBagde = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/botNoVerif.png`);
        }

        const slashBadge = await read(`${process.cwd()}/assets/discord-arts/Images/profileImage/Badges/v14/SlashBot.png`);
